...
const Projects = () => {
  ...
  const handleEvidenceSubmit = (files: File[], description: string) => {
    if (addEvidenceProjectId) {
      setProjects(projects.map(project => {
        if (project.id === addEvidenceProjectId) {
          ...
          return {
            ...project,
            evidence: [...project.evidence, ...newEvidence]
          };
        }
        return project;
      }));
  ...
  const handleUpdateSubmit = () => {
    if (selectedProjectIdForUpdate && newUpdateContent) {
      const newUpdate = { date: new Date(), content: newUpdateContent };
      setProjects(projects.map(project => {
        if (project.id === selectedProjectIdForUpdate) {
          return {
            ...project,
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        ...
        <Button className="mt-4 md:mt-0" onClick={() => setIsCreatingProject(true)}>
      ...
      <Dialog open={isAddingUpdate} onOpenChange={setIsAddingUpdate}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdateSubmit}>Save Update</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {/* Evidence Dialog */}
      <EvidenceDialog 
        open={addEvidenceProjectId !== null} 
        onOpenChange={() => setAddEvidenceProjectId(null)}
        onSave={handleEvidenceSubmit}
      />
          ...
          {selectedProject && (
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <h3 className="font-medium mb-1">Spent</h3>
                  <p className="text-sm">{formatCurrency(selectedProject.spent)}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm">{selectedProject.endDate ? format(selectedProject.endDate, "PPP") : "Not set"}</p>
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    {selectedProject.updates.map((update, idx) => (
                      <div key={idx} className="bg-muted p-2 rounded-md">
                        <div className="text-xs text-muted-foreground">{format(new Date(update.date), "PPP")}</div>
              ...
              <div>
                ...
                {selectedProject.evidence.length > 0 ? (
                  <div className="space-y-2">
                    {selectedProject.evidence.map((item, idx) => (
